...
export async function onRequestPost({ request, env }) {
  try {
    ...
    const input = mode === "split" ? [
      ...
      "Each visualDirection is the production prompt shown in the scene card. Describe only what the image or video should visibly show: characters, action, environment, emotion, composition and continuity.",
      ...
      "Each visualDirection is the production prompt shown in the scene card. Describe the visible characters, action, environment, emotion, composition and important continuity details for that exact moment.",
      `Choose one scene-specific mood from: ${SCENE_MOODS.join(", ")}.`,
      `Choose one scene-specific camera from: ${SCENE_CAMERAS.join(", ")}.`,
